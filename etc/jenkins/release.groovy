...
//   EL_BUILD_SUPPORT_VERSION       - EclipseLink Build Support version to release
//   NEXT_EL_BUILD_SUPPORT_VERSION  - Next EclipseLink Build Support snapshot version to set (e.g. 1.2.4-SNAPSHOT)
...
"""
    ...
    environment {
        EL_BUILD_SUPPORT_DIR="${WORKSPACE}/el-maven-plugin"
    }
    ...
    stages {
        ...
        // Perform release
        stage('Build and release EclipseLink Build Support') {
            steps {
                git branch: GIT_BRANCH_RELEASE, credentialsId: SSH_CREDENTIALS_ID, url: GIT_REPOSITORY_URL
                sshagent([SSH_CREDENTIALS_ID]) {
                    container('el-build') {
                        sh '''
                        etc/jenkins/release.sh "${EL_BUILD_SUPPORT_VERSION}" "${NEXT_EL_BUILD_SUPPORT_VERSION}" "${DRY_RUN}" "${OVERWRITE_GIT}" "${OVERWRITE_STAGING}"
                    '''
                    }
                }
            }
